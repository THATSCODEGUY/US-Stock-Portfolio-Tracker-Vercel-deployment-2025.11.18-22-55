...
import { TrendingUpIcon, DollarSignIcon, WalletIcon, PencilIcon, XMarkIcon, CheckIcon, BanknotesIcon } from './icons';
...
const SummaryCard: React.FC<{ 
  ...
  onEditClick?: () => void;
}> = ({ title, value, change, changePercent, icon, isPositive, isEditable = false, onSave, onEditClick }) => {
            ...
            {isEditable && (
                <div className="flex-shrink-0">
                    {isEditing ? (
                        ...
                        <button 
                            onClick={() => onEditClick ? onEditClick() : setIsEditing(true)} 
...
export const PortfolioSummary: React.FC<PortfolioSummaryProps> = ({ data, onUpdateCash, onOpenInvestmentHistory }) => {
  const { totalMarketValue, totalGainLoss, totalGainLossPercent, tradingCash, originalInvestment } = data;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <SummaryCard 
        ...
        icon={<BanknotesIcon />}
        isEditable={true}
        onEditClick={onOpenInvestmentHistory}
      />
    </div>
  );
};