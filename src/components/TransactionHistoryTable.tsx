...
export const TransactionHistoryTable: React.FC<TransactionHistoryTableProps> = ({ transactions, onEdit, onDelete, onImport, onExport, onExportAll }) => {
  ...
  // Filter out Deposit/Withdrawal transactions for this view
  const displayTransactions = transactions.filter(tx => tx.type === 'BUY' || tx.type === 'SELL');
        ...
        <div className="bg-gray-800 rounded-lg shadow-lg overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left text-gray-300">
              ...
              <tbody>
                {displayTransactions.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(tx => (
