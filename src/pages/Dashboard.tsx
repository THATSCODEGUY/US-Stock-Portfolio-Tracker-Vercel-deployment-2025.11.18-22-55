...
export const Dashboard: React.FC = () => {
    ...
    const calculatePositions = (transactions: Transaction[]): { [key: string]: { shares: number, cost: number, companyName: string } } => {
        ...
        transactions.forEach(tx => {
            // ISOLATION: Ignore DEPOSIT and WITHDRAWAL for stock position calculations
    ...
    const summaryData = useMemo(() => {
        ...
        // ISOLATION: Calculate Original Investment purely from DEPOSIT/WITHDRAWAL transactions
        const originalInvestment = activeTransactions.reduce((acc, tx) => {
            if (tx.type === 'DEPOSIT') return acc + (tx.shares * tx.price);
            if (tx.type === 'WITHDRAWAL') return acc - (tx.shares * tx.price);
            return acc;
        }, 0);
    ...
    }, [positions, activeAccount, activeTransactions]);
    ...
    // Helper for Adding Original Investment Entries (Isolated from Cash)
    const handleAddInvestmentEntry = async (date: string, type: 'DEPOSIT' | 'WITHDRAWAL', amount: number, notes: string) => {
        if (!activeAccount || !portfolioData || !user) return;

        // We use Ticker='USD' and Shares=1, Price=Amount as a convention for cash records
        // Note: We DO NOT update account.cash here based on user requirements for isolation.
        const { data: insertedTx, error: txInsertError } = await supabase
            .from('transactions')
            .insert({
                user_id: user.id,
                account_id: activeAccount.id,
                ticker: 'USD', 
                company_name: type === 'DEPOSIT' ? 'Deposit' : 'Withdrawal',
                type: type,
                shares: 1, 
                price: amount,
        ...
        if (txInsertError || !insertedTx) return console.error("Failed to add investment entry:", txInsertError?.message);
    ...
    // Helper for Deleting Original Investment Entries (Isolated from Cash)
    const handleDeleteInvestmentEntry = async (tx: Transaction) => {
         if (!activeAccount || !portfolioData) return;
         
         const { error: deleteError } = await supabase.from('transactions').delete().eq('id', tx.id);
         if (deleteError) return console.error("Failed to delete investment entry:", deleteError.message);

         const updatedTransactions = activeTransactions.filter(t => t.id !== tx.id);
         setPortfolioData(prev => ({
             ...prev!,
        ...
        <div className="min-h-screen bg-gray-900 text-gray-200">
            ...
            <main className="container mx-auto p-4 md:p-8 space-y-8">
                <PortfolioSummary 
                    ...
                    onOpenInvestmentHistory={() => setIsInvestmentHistoryOpen(true)}
                />
            ...
            <InvestmentHistoryModal
                isOpen={isInvestmentHistoryOpen}
                onClose={() => setIsInvestmentHistoryOpen(false)}
                transactions={activeTransactions}
                onAdd={handleAddInvestmentEntry}
                onDelete={handleDeleteInvestmentEntry}
            />
